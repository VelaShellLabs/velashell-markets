...
**统一认证服务已于 2026-08-30 拆到独立仓库 [velashell-identity](https://github.com/joesdu/velashell-identity)** ——
它不再只服务插件市场(资讯服务等也在用)。本仓库是**资源服务器**:只验令牌,不发令牌,
也不再自带认证服务。本地开发要先把那个仓库起起来,否则登录会失败。
